...
public class RookRules {
    ...
    public boolean noPieceIsBlocking(Move move, Board board) {
        int i; //i is smaller of the start and end squares
        int j;
        ...
        for (i++; i < j; i++) { //Loop over all squares the rook moves over and check if they are empty
            if (axis && board.getBoard()[move.getMove()[1]][i] != 0) { //Movement happens in x-axis
                return false;
            } else if (!axis && board.getBoard()[i][move.getMove()[0]] != 0) { //Movement happens in y-axis
                return false;
            }
        }
        return true;
